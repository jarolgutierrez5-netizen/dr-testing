...
// Tiers a pitcher's (mock) HR/9 for the Pitcher Report filter buttons. Thresholds are
// a rough split around league average (LEAGUE_AVG_HR9 = 1.2), not a precise model.
export function classifyPitcherHr9(hr9) {
  if (hr9 <= 0.9) return { key: "stingy", text: "🛡️ Stingy", tone: "green" };
  if (hr9 >= 1.5) return { key: "prone", text: "⚠️ Gopher-Prone", tone: "amber" };
  return { key: "average", text: "➖ Average", tone: "slate" };
}
export const PITCHER_HR9_LABELS = [
  { key: "stingy", text: "🛡️ Stingy" },
  { key: "average", text: "➖ Average" },
  { key: "prone", text: "⚠️ Gopher-Prone" },
];
