...
import { getLiveStandings, getLiveSlate } from "./liveStore";

// getStandings/getTeam/getSlate prefer live data (hydrated client-side from
// /api/standings, backed by the MLB Stats API -- see lib/providers/mlbStatsApi.js)
// and fall back to the static mock/sample data in /data until that first fetch
// resolves, or if it fails. Every other getter below still reads static data --
// player-level stats/injuries/starters aren't wired to a live source yet.
export function getStandings() { return getLiveStandings() || Object.values(TEAMS); }
export function getTeam(abbr) {
  const live = getLiveStandings();
  if (live) return live.find(t => t.abbr === abbr) || TEAMS[abbr];
  return TEAMS[abbr];
}
export function getSlate() { return getLiveSlate() || SLATE; }
