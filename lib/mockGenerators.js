...
import {
  PITCH_TYPES, PITCH_VELO_BASE, LEAGUE_AVG_K_PCT, LEAGUE_AVG_SB_PER_G, LEAGUE_AVG_HR9,
  LEAGUE_AVG_ERA, LINEUP_POSITIONS, MOCK_SURNAMES, ZONE_ROWS, ZONE_COLS,
} from "./constants";
...
// Home runs allowed per 9 innings -- MOCK DATA. Unlike K/9 and WHIP (both computed
// directly from real last-start box score fields), there's no home-runs-allowed field
// anywhere in the pitcher data, so this can't be a real derived stat. Deterministically
// correlated with the pitcher's real last-start ERA (worse ERA -> a bit more
// homer-prone) rather than independently random, same convention as
// mockAdvancedPitchingMetrics's K%.
export function mockPitcherHr9(pitcherName, eraHint) {
  const rand = seededRandom(pitcherName + ":hr9");
  const jitter = (rand() - 0.5) * 0.6;
  return clamp(LEAGUE_AVG_HR9 + (eraHint - LEAGUE_AVG_ERA) * 0.18 + jitter, 0.2, 2.8);
}
