...
// Real, derived from a pitcher's last-start line (PITCHERS in /data) -- every entry
// there always has ip/k/bb/h, so these never hit missing-data cases the way season
// stats (PITCHER_SEASON_STATS) can. Used both to seed the K% mock generator (K/9 is a
// real anchor for otherwise-mock advanced metrics) and as Pitcher Report sort keys.
export function pitcherK9(pi) { return (pi.k / pi.ip) * 9; }
export function pitcherWhip(pi) { return (pi.bb + pi.h) / pi.ip; }
...
// only still needed as a fallback for the static mock SLATE (data/slate.js), which has
// no real UTC timestamp. The live MLB Stats API feed (lib/providers/mlbStatsApi.js)
// supplies a real `gameDate` ISO timestamp per game, which minutesUntilFirstPitch below
...
// Accepts either a slate game object ({ time, gameDate? }) or a bare "H:MM AM/PM" string
// for backward compatibility with the static mock slate.
export function minutesUntilFirstPitch(gameOrTimeStr) {
  const dt = typeof gameOrTimeStr === "string"
    ? parseGameTime(gameOrTimeStr)
    : gameOrTimeStr.gameDate ? new Date(gameOrTimeStr.gameDate) : parseGameTime(gameOrTimeStr.time);
  return (dt - new Date()) / 60000;
}

// Display-only game time. The live MLB Stats API provider sets `time` to a string
// already formatted in the *server's* timezone (see lib/providers/mlbStatsApi.js) --
// fine as a fallback, but wrong for anyone not in that timezone. Whenever `gameDate`
// (the real ISO UTC timestamp) is present, this reformats it in the caller's own
// timezone instead -- meant to be called client-side, where "caller's timezone" means
// the viewer's, not the server's.
export function formatGameTime(game) {
  if (game.gameDate) {
    return new Date(game.gameDate).toLocaleTimeString([], { hour: "numeric", minute: "2-digit", timeZoneName: "short" });
  }
  return game.time;
}
...
export function getGameLockInfo(gameLabel) {
  ...
  return { scheduled: true, locked: minutesUntilFirstPitch(match) <= LOCK_WINDOW_MINUTES, time: match.time };
}
